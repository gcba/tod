...
// descargar data
function create_download_data(layer) {
    $("#button-download-data-divisions").click(function() {
        var query = layer.getSubLayer(SUBLAYER_IDX["divisions"]).getSQL()
        do_db_query(query, function(data) {
            if (data == '')
                return;
            // debugger
            ...
            JSONToCSVConvertor(rows, "DivisionesData", true);
        })
    })

    $("#button-download-data-buffers").click(function() {
        var query = layer.getSubLayer(SUBLAYER_IDX["buffers"]).getSQL()
        // debugger
        do_db_query(query, function(data) {
            if (data == '')
                return;
            ...
            JSONToCSVConvertor(rows, "BuffersData", true);
        })
    })
}
